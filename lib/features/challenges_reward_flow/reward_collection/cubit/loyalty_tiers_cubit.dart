...
class LoyaltyTiersCubit extends Cubit<LoyaltyTiersState> {
  ...
  Future<void> load() async {
    emit(state.copyWith(status: LoyaltyTiersLoadStatus.loading, errorMessage: ''));
    final result = await _repository.getTiers();
    switch (result) {
      case ApiSuccess(:final data):
        final sorted = [...data]..sort((a, b) => a.sortOrder.compareTo(b.sortOrder));
        emit(
          state.copyWith(
            status: LoyaltyTiersLoadStatus.loaded,
            tiers: sorted,
          ),
        );
